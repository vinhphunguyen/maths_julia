...
markangle("$\alpha$",O,C,B,n=2,radius=6mm);
markangle("$\alpha-\beta$",C,O,F,n=1,radius=17mm);
...
draw(Label("$\beta$",MidPoint,Fill(white)),arc((0,0),0.26,0,degrees(alpha)),black+1.pt);
draw(Label("$\beta$",MidPoint,Fill(white)),arc(B,0.2,90,90+degrees(alpha)),black+1.pt);

//label(pic1,"$x+2y=2$",(1.2,1));

Label L1=Label("$1$",align=(0.,.0),MidPoint);
Label L2=Label("$\sin\alpha$",align=(0.,.0),MidPoint);
Label L3=Label("$\cos\alpha$",align=(0.,.0),MidPoint);
label(rotate(degrees( alpha+beta ))*L1, (0.2,B.y));
label(rotate(degrees( alpha ))*L2, (0.4,.3));
label(rotate( degrees(alpha)-90)*L3, (0.58,.6));

plot_label(O,A,D,"$\sin\alpha\cos\beta$",.07);
plot_label(A,B,O,"$\sin\alpha\cos\beta$",.07);
plot_label(B,D,C,"$\cos\beta\cos\alpha$",.07);
plot_label(O,F,B,"$\cos(\alpha-\beta)$",.07);
plot_label(F,C,B,"$\sin(\alpha-\beta)$",.07);
plot_label(C,D,A,"$\cos\alpha\sin\beta$",.07);
