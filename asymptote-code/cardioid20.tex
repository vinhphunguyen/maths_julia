...
% \pgfmathparse{mod(1*2+1,10)} \pgfmathresult %displays 2.0
% \pgfmathtruncatemacro{\myint}{ \pgfmathresult}  

% \myint %displays 2 

\begin{tikzpicture} 
  \pgfmathsetmacro{\np}{200}  % number of points on the circle
  \pgfmathsetmacro{\as}{360/\np}  % angle
  \pgfmathsetmacro{\ea}{360-\as}
  \pgfmathsetmacro{\rnp}{\np-1}
  \pgfmathsetmacro{\rad}{2.4} % radius of the circle
  \pgfmathsetmacro{\radd}{2.8} % radius of the circle
  \pgfmathsetmacro{\rr}{0.05} % radius of the dots (points)
  \pgfmathsetmacro{\num}{2.5} % time table of num

  \draw [thick] (0,0) circle (\rad);
    % loop over 0, as, 2as, ..., 360
    % get coordinates using polar coordinate, r = 3.5cm
  \foreach \x [count=\i from 0] in {0,\as,...,\ea}{
    %\pgfmathsetmacro{\td}{int(\i+1)}
    \coordinate (\i) at (\x:\rad) {};
    %\node[] at (\x:\radd) {\i};
  }
 
  \foreach \x in {0,1,2,...,\rnp}{
    \pgfmathsetmacro{\nx}{mod(\num*\x,\np)} %thefis=1
    %\stepcounter{fis}
    \draw[black] (\x) -- (\nx);
  } 

  % \foreach \i in {0,1,...,\rnp}{
  %     \draw[black,fill=black] (\i) circle (\rr);
  % }
\end{tikzpicture}
