...
do
{
   ...
   string [] sdots = new string[17];

   string s   = (string) prod;
   string[] ss = array(s);

   for(int jj=0; jj<length(s); ++jj)
   {
     sdots[16-jj] =  ss[length(s)-jj-1];
   }

   for(int jj=0; jj<17-length(s); ++jj)
   {
   sdots[jj] = "";//Xóa
   }
   string sdotss = "";
   ...
   draw(pic2,(0,-i*dy-.02)--(2,-i*dy-.02));
   label(pic2, L2, (2.038,-i*dy-.02),W);
   if ( i == 1 )
   {
     draw(pic2,(0,-.02)--(2,-.02));
     label(pic2, L2, (2.038,-.02),W);
   }
   // next row
   i = i - 1;
} while ( i != 0 );
