...
\begin{tikzpicture}[scale=1]
  ...
  \draw[fill=cyan] (o)--(f)--(q)--(e)--cycle;

  \draw[thick] (o)--(a);
  \draw[thick] (o)--(b);
  \draw[thick] (o)--(c);
  \draw[thick,latex-] (p)--(q);
  \draw[line width=.2mm, -latex] (o)--(p);
  \draw[line width=.2mm, -latex] (o)--(q);

  \node[left] at (o) {$O$};
  \node[right] at (a) {$x$};
  \node[right] at (b) {$z$};
  \node[right] at (c) {$y$};
  \node[left] at (1,1.5) {$\vb*{u}$};
  \node[right] at (2,1.5) {$\vb*{u}-\Pi_{(i,j)}(\vb*{u})$};
