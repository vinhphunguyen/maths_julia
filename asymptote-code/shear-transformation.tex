...
\fill[orange!20!yellow] (0,0)--(1,0)--(2,1)--(1,1);

% grid on a rectangle (-7,-3) to (7,3), step=0.5
\draw[step=1cm, gray!20!white, very thin] (\xa, \ya) grid (\xb, \yb);
% shear transformation (x+y,y), y =-2, and 2, to get two points on the transformed lines
\foreach \x in {-4,...,4} {
    \draw[color=red, thin] (-3 + \x, -3) -- (3 + \x, 3);
}

\foreach \y in {-3,...,3} {
    \draw[color=red, thin] (-5, \y) -- (5, \y);
}
...
\draw[color=red, line width=1.10pt, ->] (0, 0) -- (1, 0) node[below] {$\hat{\vb*{i}}$};
\draw[color=blue, line width=1.10pt, ->] (0, 0) -- (1, 1)node[above left] {$\hat{\vb*{j}}$};
