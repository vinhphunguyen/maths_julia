...
# plot the binomial distribution with vertical lines
function plot_binomial_distribution(n,p,a,b,filename)
    xs  = range(1,n,step=1)
    ys  = zeros(length(xs))
    ys  = zeros(length(xs))
    for i =1:n
...
#plot_binomial_distribution(50,0.4,0,60,"plot_binomial_distribution50.pdf")
plot_binomial_distribution(200,0.4,50,110,"plot_binomial_distribution100.pdf")
...
#plot_binomial_distribution_normal(55,0.3,"binomial_vs_moivre2.pdf")
...
function plot_2_binomial_distribution(n, p1, p2, filename)
    xs = range(1, n, step=1)
    ys1 = zeros(length(xs))
    ys2 = zeros(length(xs))
    for i = 1:n
        ys1[i] = binomial(big(n), i) * (p1)^(i) * (1 - p1)^(n - i)
        ys2[i] = binomial(big(n), i) * (p2)^(i) * (1 - p2)^(n - i)
    end
    fig, ax = plt.subplots(1, 1, figsize=set_size())
    ax.spines["right"].set_visible(false)
    ax.spines["top"].set_visible(false)
    ...
    ax.set_xticks(0:11)
    #ax.set_yticks([0,0.25,0.5])
    plt.bar(xs, ys1, color="blue", width=1.0,label=L"p=0.5")
    plt.bar(xs, ys2, color="orange", alpha=0.4, width = 0.8,label=L"p=0.8")
    plt.xlabel(L"$x$")
    plt.ylabel(L"$P_X(x)$")
    plt.ylim(bottom=0)
    plt.xlim([0, n])
...
n = 10; p1 = 0.5; p2 = 0.8; filename = "plot_2_binomial_distribution.pdf"
plot_2_binomial_distribution(n, p1, p2, filename)
